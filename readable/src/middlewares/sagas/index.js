...
function* requestAllPosts() {
	try {
		...
		yield delay(1000);
		const [ posts, categories ] = yield all([ call(PostsAPI.getAll), call(PostsAPI.getCategories) ]);
		yield put({
			type: SUCCESS_POSTS,
...
function* addNewComment({ comment, parentId }) {
	try {
		const response = yield call(PostsAPI.addComment, formatComment({ ...comment, parentId }));
...
function* updateComment({ id, body }) {
	try {
		const response = yield call(PostsAPI.updateComment, { id, body, timestamp: Date.now() });
		yield put({
